...
- **有组织形态**——组是隔离与切换单位：默认组是这套智械集群，你也可以按公司架构、团队架构
  组建任意多个组，甚至只放一个主智能体，让它自己扩编。
...
| 独立智能体 | 除组之外还可创建独立个体（预置 Machina），不经调度直接服务——单体与集群共用一套记忆与模型体系 |
...
- **交互目标**——你对话的对象：某个**智能体组**（集群协作）或某个**独立智能体**（单体服务）。
...
```bash
exm group create 公司架构组 --id company --description "经理 → 员工"
